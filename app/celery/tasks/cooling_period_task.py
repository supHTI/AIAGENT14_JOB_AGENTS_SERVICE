...
#     Emails include Excel attachments with detailed candidate information.
...
#         # Fetch candidates with joined status - including job and clawback details
...
#                 CandidateJobs.id.label("candidate_job_id"),
#                 CandidateJobStatus.id.label("status_id"),
#                 CandidateJobStatus.cooling_period_closed,
#                 CandidateJobStatus.joined_at,
#                 JobOpenings.title.label("job_title"),
...
#         # Group candidates by assigned_to user and job title
#         user_candidates = defaultdict(lambda: defaultdict(list))
        
#         for row in joined_candidates_data:
#             remaining_days = None
#             clawback_end_date = None
            
#             if row.joined_at and row.cooling_period:
#                 clawback_end_date = row.joined_at + timedelta(days=int(row.cooling_period))
...
#                 "clawback_end_date": clawback_end_date.date() if clawback_end_date else None,
#                 "days_remaining": remaining_days,
#                 "cooling_period_remaining_days": remaining_days,
#             }
            
#             job_title = row.job_title or "Unknown Job"
#             user_candidates[row.assigned_to][job_title].append(candidate_info)
...
#         for user_id, jobs_dict in user_candidates.items():
#             # Fetch user details
...
#             # Combine all candidates for this user
#             all_candidates = []
#             for job_title, candidates in jobs_dict.items():
#                 all_candidates.extend(candidates)
            
#             # Generate Excel file for this user (one sheet per job title)
#             try:
#                 if len(jobs_dict) == 1:
#                     # Single job - generate single sheet Excel
#                     job_title = list(jobs_dict.keys())[0]
#                     excel_data = ExcelGenerator.generate_hr_level_excel(job_title, jobs_dict[job_title])
#                 else:
#                     # Multiple jobs - we'll create multiple sheets in one Excel
#                     # For now, using the first job as primary
#                     job_title = list(jobs_dict.keys())[0]
#                     excel_data = ExcelGenerator.generate_hr_level_excel(f"All Jobs", all_candidates)
...
#                 logger.info(f"Email {'sent' if success else 'failed'} to {user.email} with {len(all_candidates)} candidate(s)")
...
#     Includes Excel attachments with detailed information.
...
#         # Fetch candidates with joined status - including clawback details
#         joined_candidates_data = (
#             db.query(
#                 Candidates.candidate_id,
#                 Candidates.candidate_name,
...
#             .filter(CandidateJobStatus.cooling_period_closed.is_(None))  # Only active cooling periods
#             .all()
#         )
        
#         logger.info(f"Found {len(joined_candidates_data)} joined candidates for admin summary")
        
#         # Group candidates by assigned HR user and job title
#         hr_candidates = defaultdict(lambda: defaultdict(list))
...
#             hr_candidates[hr_key][job_title].append({
...
#         # Prepare summary data for admins (grouped by HR)
#         summary_data = []
#         for hr_key, jobs_dict in hr_candidates.items():
#             all_candidates = []
#             hr_info = None
            
#             for job_title, candidates_data in jobs_dict.items():
#                 if not hr_info and candidates_data:
#                     hr_info = candidates_data[0]
                
#                 all_candidates.extend([c["candidate"] for c in candidates_data])
...
#                 excel_data = ExcelGenerator.generate_manager_level_excel(
#                     "Cooling Period - All Jobs",
#                     summary_data
#                 )
